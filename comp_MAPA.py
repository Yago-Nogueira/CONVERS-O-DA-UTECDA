...
class COMP_MAPA(QDialog):	
	def __init__(self, filedir, data_inicio, data_fim , h_interval, siglas, extend_LAT_LONG, var_barra_progess, var_barra_progess_label, dado_config,Coordenadas_equador_Magnetico_X,Coordenadas_equador_Magnetico_Y, Value_Check_VIDEO_MAPA, stop_thread_MAPA,varx,Entry_MAPA_Xtick,vary,Entry_MAPA_Ytick, varabs):
		...
		self._Value_Check_VIDEO_MAPA = Value_Check_VIDEO_MAPA
		...
		self._stop_thread_MAPA = stop_thread_MAPA
		self._varx=varx
		self._Entry_MAPA_Xtick=Entry_MAPA_Xtick
		self._vary=vary
		self._Entry_MAPA_Ytick=Entry_MAPA_Ytick
		self._varabs=varabs
		...
		self._PERIODO_MAPA = date_range(start = self._data_inicio, end = self._data_fim, freq=('%sT'%self._h_interval))
		self._PERIODO_MAPA_dias = date_range(start = self._data_inicio.date(), end = self._data_fim.date(), freq='D')
	...
	def _crate_video(self, dirPNG):
		images = [os.path.join(dirPNG, img) for img in os.listdir(dirPNG) if img.endswith(".png")]
		images.sort()
		videoDir = os.path.join(dirPNG,dirPNG.split("\\")[-1] + ".gif")
		with get_writer(videoDir, mode='I') as writer:
			for filename in images:
				image = imread(filename)
				writer.append_data(image)
		
		clip = VideoFileClip(videoDir)
		clip.write_videofile(videoDir.replace("gif","mp4"))
		clip.close()


		# video_name = dirPNG.split("\\")[-1] + ".mp4"
		# images = [os.path.join(dirPNG, img) for img in os.listdir(dirPNG) if img.endswith(".png")]
		# images.sort()
		# frame = cv2.imread(os.path.join(dirPNG, images[0]))
		# height, width, layers = frame.shape
		# video = cv2.VideoWriter(video_name, 0, 1, (width,height))
		# for image in images:
		# 	video.write(cv2.imread(os.path.join(dirPNG, image)))
	...
	def _set_Matplotlib_grafico_VTEC(self):
		...
		font = self._dado_config.get_font_Settings("MAPA")		
		""" 
...
#'____________________________________________________________________________________________________________________________'
		...
		self.list_prn = {}
		self.nome_est = {}
		self.coef_loop_leitura = ((100)/(3*len(self._siglas)*len(self._PERIODO_MAPA_dias)))
...
#'____________________________________________________________________________________________________________________________'
		...
		_ticks_cbar = self._dado_config.Settings["MAPA"]["iTicksCbar_VTEC"]
		_ticks_divisao = self._dado_config.Settings["MAPA"]["iDivTicks_VTEC"]
		_vm_max = self._dado_config.Settings["MAPA"]["fValueMax_B_VTEC"]
		_vm_min = self._dado_config.Settings["MAPA"]["fValueMin_B_VTEC"]
		...
		self.coef_loop_processamento = ((100)/(3*len(self._siglas)*len(self._PERIODO_MAPA_dias)))
		for MAPA_dia in self._PERIODO_MAPA_dias:
			MAPA_dia_date = MAPA_dia.date()
			if self.nome_est[MAPA_dia_date]:		
				...
				try:
					self.pasta[MAPA_dia_date] = ("(VMAX %.2f_%.2f) Contorno_VTEC_%s-%.2i-%.2i"%(_vm_min,_vm_max,MAPA_dia_date.year,MAPA_dia_date.month,MAPA_dia_date.day))
		...
		for PERIODO_DATA_MAPA in self._PERIODO_MAPA:
			try:
				self.fig.clf()
				axes = self.fig.add_subplot(111, projection = PlateCarree())
				# hora+=0.00001
				# horas = int(hora)
				# minutos = (hora*60) % 60
				...
				axes.add_feature(cfeature.BORDERS, linewidth=2.0)
				axes.add_feature(cfeature.COASTLINE, linewidth=2.0)
				x, y, z = np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.lon']), np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.lat']), np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.vtec']) 
				...
				#scitools.org.uk/cartopy/docs/v0.13/matplotlib/gridliner.html
				gl = axes.gridlines(crs=ccrs.PlateCarree(),draw_labels=True)				
				gl.xlabels_top=False
				gl.ylabels_right=False
				if self._varx.get()==1:
                                        t=self._Entry_MAPA_Xtick.get()
                                        x=t.split(",")
                                        for i in range(len(x)):
                                            x[i]=int(x[i])
                                        gl.xlocator = mticker.FixedLocator(x)
				if self._vary.get()==1:
                                        t=self._Entry_MAPA_Ytick.get()
                                        y=t.split(",")
				...
				cbar_mapa.ax.set_title(_titulo_bar,pad=15,**font)
				camff = (("%s\\%s\\%s.png")%(self._filedir,self.pasta[PERIODO_DATA_MAPA.date()],hora_HMS.replace(':',"-")))
				self.fig.subplots_adjust(
                    top=self._dado_config.Settings["MAPA"]["fTop"],
                    bottom=self._dado_config.Settings["MAPA"]["fBottom"],
                    left=self._dado_config.Settings["MAPA"]["fLeft"],
                    right=self._dado_config.Settings["MAPA"]["fRight"],
                    hspace=self._dado_config.Settings["MAPA"]["fHspace"],
                    wspace=self._dado_config.Settings["MAPA"]["fWspace"]
                )
				# print(camff)
		...
		plt.cla()
		plt.clf()
		if self._Value_Check_VIDEO_MAPA:
			self._var_barra_progess_label.set(self._dado_config.idioma(192))
			self._crate_video(("%s\\%s")%(self._filedir,self.pasta[PERIODO_DATA_MAPA.date()]))
	...
	def _thread_organizar_dados_ROT(self,q, MAPA_dia_date):
		while not q.empty():
			...
			for prn in _prn:
				str_prn = str(prn)
				self.dados_organizados[MAPA_dia_date][_est][str_prn] = self.uti.get_ROT(self.dados_organizados[MAPA_dia_date][_est][str_prn], delta_time_rot = self._dado_config.Settings["MAPA"]["fValueDelta_ROT"], intervalo_LEITURA_CMN = 30)
				# self.dados_organizados[MAPA_dia_date][_est][str_prn] = self.uti.get_ROTI(self.dados_organizados[MAPA_dia_date][_est][str_prn], delta_time_roti = self._dado_config.Settings["MAPA"]["fValueDelta_ROTI"], intervalo_LEITURA_CMN = 30)
				for hora in self.dados_organizados[MAPA_dia_date][_est][str_prn].keys():  
					f_hora = hora
					if not np.isnan(self.dados_organizados[MAPA_dia_date][_est][str_prn][f_hora]['rot']):
						...
						try:
                                                        if self._varabs.get()==1:
                                                                ...
                                                                self.dados_organizados_plot[MAPA_dia_date][f_hora+".rot"].append(self.dados_organizados[MAPA_dia_date][_est][str_prn][f_hora]['rot'])
						except KeyError:
	...
	def _set_Matplotlib_grafico_ROT(self):
		_titulo_bar = self._dado_config.Settings["MAPA"]["sTitle_B_ROT"]
...
#'____________________________________________________________________________________________________________________________'
		...
		_ticks_cbar = self._dado_config.Settings["MAPA"]["iTicksCbar_ROT"]
		_ticks_divisao = self._dado_config.Settings["MAPA"]["iDivTicks_ROT"]
		_vm_max = self._dado_config.Settings["MAPA"]["fValueMax_B_ROT"]
		_vm_min = self._dado_config.Settings["MAPA"]["fValueMin_B_ROT"]
		levels = np.linspace(_vm_min,_vm_max,int(_ticks_cbar + ((_ticks_cbar-1)*(_ticks_divisao-1))))
		ticks = np.linspace(_vm_min,_vm_max,int(_ticks_cbar))
		...
		# nmap = mpl.cm.ScalarMappable(norm=norm, cmap=self.cmap)

		# arq_TESTE = open("C:\\Users\\mateu\\Desktop\\saida_ROT.txt", 'w')

		#'____________________________________________________________________________________________________________________________'
		self._var_barra_progess_label.set(self._dado_config.idioma(179))
		#'____________________________________________________________________________________________________________________________'
		self.coef_loop_plot = ((100)/(3*len(self._PERIODO_MAPA)))
		for PERIODO_DATA_MAPA in self._PERIODO_MAPA:
			try:
				self.fig.clf()
				axes = self.fig.add_subplot(111, projection = PlateCarree())
				...
				axes.add_feature(cfeature.BORDERS, linewidth=2.0)
				axes.add_feature(cfeature.COASTLINE, linewidth=2.0)
				x, y, z = np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.lon']), np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.lat']), np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.rot']) 
				# scx,scy = x, y
				xi = np.linspace(x.min(), x.max(), self.numcols)
				yi = np.linspace(y.min(), y.max(), self.numrows)
				xi, yi = np.meshgrid(xi, yi)
				...
				COLOR_SET = axes.contourf(
					xi,yi,GD1,
					cmap = self.cmap, #CMAP_GRAFICO_MAPA_ROT
					extend='both',
					levels = levels,
					transform=PlateCarree()
				)
				...
				#self.fig.set_size_inches(self._dado_config.Settings["MAPA"]["fSize_inches_fig_width"], self._dado_config.Settings["MAPA"]["fSize_inches_fig_height"])
				self.fig.savefig(camff,dpi=self.fig.dpi)
				self._var_barra_progess_label.set("Figura - "+camff+" gerada")
			except (Exception) as e: 
				print(e)
...
#'____________________________________________________________________________________________________________________________'
		...
		for MAPA_dia in self._PERIODO_MAPA_dias:
			...
			if self.nome_est[MAPA_dia_date]:
				...
				try:
					self.pasta[MAPA_dia_date] = ("(VMAX %.2f) Contorno_ROTI_%s-%.2i-%.2i"%(_vm,MAPA_dia_date.year,MAPA_dia_date.month,MAPA_dia_date.day))
					os.makedirs(self._filedir+"\\"+self.pasta[MAPA_dia_date])
				except FileExistsError:
					pass

	
		_ticks_cbar = self._dado_config.Settings["MAPA"]["iTicksCbar_ROTI"]
		_ticks_divisao = self._dado_config.Settings["MAPA"]["iDivTicks_ROTI"]
		_vm_max = self._dado_config.Settings["MAPA"]["fValueMax_B_ROTI"]
		_vm_min = self._dado_config.Settings["MAPA"]["fValueMin_B_ROTI"]
		passo=int(_ticks_cbar + ((_ticks_cbar-1)*(_ticks_divisao-1)))
		levels = np.linspace(_vm_min,_vm_max,int(_ticks_cbar + ((_ticks_cbar-1)*(_ticks_divisao-1))))
		ticks = np.linspace(_vm_min,_vm_max,int(_ticks_cbar))
		#_ticks_cbar = 10
		#_ticks_divisao = 4
		#passo = _vm/(_ticks_cbar*_ticks_divisao)
		bounds = ticks#np.arange(.1,_vm+passo,passo)
		#ticks = np.arange(0-passo,_vm+2*passo,passo)
		ticks_colorbar =  ticks#np.arange(0,_vm+passo,0.1)
		norm = colors.BoundaryNorm(bounds, self.cmap.N)
		nmap = cm.ScalarMappable(norm=norm, cmap=self.cmap)
		#'____________________________________________________________________________________________________________________________'
		self._var_barra_progess_label.set(self._dado_config.idioma(179))
		#'____________________________________________________________________________________________________________________________'
		self.coef_loop_plot = ((100)/(3*len(self._PERIODO_MAPA)))
		for PERIODO_DATA_MAPA in self._PERIODO_MAPA:
			try:
				...
				axes.add_feature(cfeature.BORDERS, linewidth=2.0)
				axes.add_feature(cfeature.COASTLINE, linewidth=2.0)
				x, y, z = np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.lon']), np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.lat']), np.array(self.dados_organizados_plot[PERIODO_DATA_MAPA.date()][f_hora+'.roti']) 
				# scx,scy = x, y
				xi = np.linspace(x.min(), x.max(), self.numcols)
				yi = np.linspace(y.min(), y.max(), self.numrows)
				xi, yi = np.meshgrid(xi, yi)
