...
#####################|Propriedades embutidas no gráfico gerado|#######################################################################################################################
#####################|Alterar por escrever o arquivo|#######################################################################################################################
...
# #####################|GRÁFICO DAS EST|#######################################################################################################################
# 		self.filedir = None#r"C:\Users\Mateus_Pillat\Google Drive\Estágio\Dados\STD"
# #####################|INICIO p1|#######################################################################################################################
...
#####################|CRIAR MAPA DO  OBS|#######################################################################################################################
		# try:
		# 	with open((('%s/OBS.dat')%(os.path.expanduser('~/Documents/UTECDA'))), 'r',encoding='utf-8') as arquivoOBS:
		# 		self.dados = [];self.est=[]
		# 		for obs in arquivoOBS:
		# 			obs_s = obs.split("\t")
		# 			# if obs_s[2] in self.est_equals :
		# 				# print(obs_s[2])
		# 			one=obs_s[3].split(' ')
		# 			two=obs_s[4].split(' ')
		# 			line_one = self.uti.DMDDEC(int(one[0]),int(one[1]))
		# 			line_two = self.uti.DMDDEC(int(two[0]),int(two[1]))
		# 			self.dados.append([line_one,line_two])
		# 			inclinacao = float((self.uti.get_inclinacao_D(float(300),int(2018),line_one,line_two)))
		# 			diplat = (('%.2f')%(math.degrees(math.atan(((math.tan(math.radians(inclinacao))/2))))))
		# 			self.est.append(obs_s[2])#+' ['+ diplat +']')

		# except IOError:
		# 	messagebox.showerror(self.Dado_config.idioma(49),self.Dado_config.idioma(55),parent=self)
		# arquivoOBS.close()
		# self.dados=np.array(self.dados, 'float')
		# print(self.dados)
#####################|CRIAR MAPA DO  OBS|#######################################################################################################################
#####################|CRIAR MAPA DO  OBS|#######################################################################################################################
...
###################################|Brasil|################################################################################
		# self.map = Basemap(projection='cyl', llcrnrlat=-35, urcrnrlat=7,llcrnrlon=-77, urcrnrlon=-32, resolution='c')
		# self.map = Basemap(projection='cyl', llcrnrlat=int(la[0]), urcrnrlat=int(la[1]),llcrnrlon=int(lo[0]), urcrnrlon=int(lo[1]), resolution='c')
###################################|Brasil|################################################################################
###################################|Mundo|################################################################################
...
# 		# self.map.drawmapboundary(fill_color='aqua', zorder=1)
...
###################################|click_points|################################################################################
###################################|inf_points|##################################################################################
		# self.txb = TextArea("", minimumdescent=False)
		# self.xybox=(50., 50.)
		# self.ab = AnnotationBbox(self.txb, (0,0), xybox=self.xybox, xycoords='data', boxcoords="offset points",  pad=0.3,  arrowprops=dict(arrowstyle="->"))
		# self.ax_map.add_artist(self.ab)
...
###################################|inf_points|################################################################################
		# self.fig_map.subplots_adjust(wspace=0,hspace=0)
		# self.fig_map.canvas.mpl_connect('button_press_event', self.on_click)
###################################|click_points|################################################################################
		# self.fig_map.tight_layout(pad=0)
		# self.Data_Entry.set('01/09/2017')
###################################|points|################################################################################
		# self.plot_linha_eq_mag(int(self.Data_Entry.get()[-4:]))#self.ano_indv)#datetime.strptime(self.cal_INDV.get(), '%d/%m/%Y').year)
#####################|CRIAR MAPA DO  OBS|#######################################################################################################################
#####################|CRIAR MAPA DO  OBS|#######################################################################################################################
#####################|CRIAR MAPA DO  OBS|#######################################################################################################################
#####################|CRIAR MAPA DO  OBS|#######################################################################################################################
...
######################################|Inserir items sem selecionar|######################################
    ...
    # 	#  OSError: [Errno 22] Invalid argument: 'C:-Users-Mateus_Pillat-Google Drive-Estágio-Dados-CMN\\2017-09-06 00:00:00_AMCO_PRN01.ROT'
	# 	for sat in self.prn:
...
###########################| |\/|Atriz do tipo Tripa...|##############################################################################################################################################################
	...
	# 	try:
	# 		with open(self.filedir+("\%s_%s_Matriz.Std"%([sgl[0] for sgl in self.siglas],self.titulo)),'w') as arqTri:
	...
	# 		inclinacao = self.uti.get_inclinacao(300,ano,'0','0',str(x),'0')
	...
	# 		if self.cbg_value.get() in ("Individual (STD)","ROT (CMN)"):# and not self.inf:
	...
	# 			self.plot()
	# 		elif self.cbg_value.get() in ("Setor (STD)", "Desvio (STD)", "EIA (STD)"):# and not self.inf:
	...
	def modelo_mapa(self,*event):
		...
		# self.cbar_model_map.ax.set_title("VTEC",**font_default,picker=5,gid="label_bar_graph_model_map")
	...
	# self.fig_map.canvas.mpl_connect('motion_notify_event', self.vis_info)
	...
	# 		# ano_line = askEntry(self.janelaprincipal,self.Dado_config.idioma(143),self.Data_Entry.get()[-4:],False,self.tela_graf)
	# 		# self.plot_linha_eq_mag(int(ano_line))
	# 		pass
	...
	def setar_legends(self,refresh=True):
		if self.cbg_value.get() == "Desvio (STD)":
			...
			if self.Legend.get() == False:
				...
				self.img_axes.legend(bbox_to_anchor=(1.009, 0, .1, 1.9), loc=3,ncol=1, mode="expand", borderaxespad=0.).draggable(True)#,facecolor='lightgrey')
	...
	def plot_DESVIO(self):
		if len(self.listaobs.curselection()) > 0:
			...
			self.siglas.sort(key=lambda x: float(x[5]),reverse=False)
			self.img,self.img_axes,self.titulo,self.tec_desv,self.desv,self.med,self.delta,self.temp,self.datas = COMP_DESVIO(self.img,self.filedir,self.siglas,self.Cal_Calm_days.selection_get(),dataI,dataF,self.DateFormat_DESVIO.get()).re_figura()
			...
			if self.titulo_graph_desv:
    				self.img.suptitle(self.titulo_graph_desv,**self.rfont_titulo_graph_desv)
	...
	# 		# if self.titulo_graph_setor and self.rfont_titulo_graph_setor:
	...
	# 		try:
	# 			if self.cal_SETOR_EIA_varest.get():
	# 				# self.img_axes.tick_params(axis='y', which='minor', bottom=False)
	# 				self.img_axes.yaxis.set_major_locator(ticker.FixedLocator(self.eixoG))
	# 				self.img_axes.minorticks_off()
	# 			# else:
	# 				# self.img_axes.yaxis.set_major_locator(ticker.LinearLocator())
	...
	# 			self.siglas.append([N_L[0],N_L[1],N_L[2],lat[0],lat[1],item_line[2].strip()])
	# 		self.vm = vm
	# 		self.img ,self.img_axes ,self.img_cbar ,self.titulo ,self.estacao_select_matriz,self.eixoG = COMP_SETOR(self.img,self.siglas,data,self.filedir,self.vm,self.varaxixY.get(),self.cal_SETOR_EIA_varest).re_figura()
	...
	# 	self.vm = vm
	# 	self.img,self.img_axes,self.img_cbar,self.titulo,self.TECdias,self.matrizstd,self.matrizdias_indv = COMP_INDV(self.img,sigla,dataI,dataF,self.filedir,self.DateFormat_INDV,self.vm).re_figura()
	# 	if self.matrizstd:
	# 		self.img.canvas.mpl_connect('pick_event', self.on_click_prop)
	# 		self.min_ori_Y,self.max_ori_Y = self.img_axes.get_ylim()
	...
	# 		self.min_ori_X,self.max_ori_X = self.img_axes.get_xlim()
	# 		# self.min_ori_X = #self.matrizdias_indv[0][0]
	# 		# self.max_ori_X = #self.matrizdias_indv[-1][0]
			
	# 		# if self.titulo_graph_indv and self.rfont_titulo_graph_indv:
	...
	# 		# if self.minX_indv or self.maxX_indv:
	...
	# 		# if self.titulo_bar_rot and self.rfont_titulo_bar_rot:
	# 			# self.img_cbar.ax.set_title(self.titulo_bar_rot,**self.rfont_titulo_bar_rot)
	...
	# 		# if self.passo_tickY_rot:self.img_axes.yaxis.set_major_locator(ticker.IndexLocator(base=(self.passo_tickY_rot+1), offset=self.minY_rot))
	# 		# elif self.n_rotuloY_rot:self.img_axes.yaxis.set_major_locator(ticker.LinearLocator(round(self.n_rotuloY_rot)))
	# 		# print([pr*10 for pr in np.arange(self.prn[0],self.prn[-1]+1,self.passo_tickY_rot/10)])
	...
	def plot_MAPA(self,vm=50):
		...
		# self.img,self.img = COMP_MAPA(self.img,self.filedir,self.data,vm,float(self.MAPA_Time_INICIO.get()),float(self.MAPA_Time_FIM.get()),float(self.MAPA_Delta_Time.get()),self.conteudo_lista).re_figura()
		self.la = self.ax_map.get_ylim()
		self.lo = self.ax_map.get_xlim()
		# COMP_MAPA(self.filedir,self.data,vm,float(self.MAPA_Time_INICIO.get()),float(self.MAPA_Time_FIM.get()),float(self.MAPA_Delta_Time.get()),self.conteudo_lista,self.titulo_bar_model_map,self.rfont_titulo_bar_model_map,self.la[0],self.la[1],self.lo[0],self.lo[1])#.re_figura()
...
#####################|Thread Mapa|#######################################################################################################################################
		...
		thread_MAPA.start()
#####################|Thread Mapa|#######################################################################################################################################
		# thread_MAPA = _thread.start_new_thread(COMP_MAPA,(self.filedir,self.data,vm,float(self.MAPA_Time_INICIO.get()),float(self.MAPA_Time_FIM.get()),float(self.MAPA_Delta_Time.get()),self.conteudo_lista,self.titulo_bar_model_map,self.rfont_titulo_bar_model_map,self.la[0],self.la[1],self.lo[0],self.lo[1],self.var_barra_loading,self.var_barra_loading_lbl, ))
		# thread_MAPA = Thread(target = COMP_MAPA, args = (self.filedir,self.data,vm,float(self.MAPA_Time_INICIO.get()),float(self.MAPA_Time_FIM.get()),float(self.MAPA_Delta_Time.get()),self.conteudo_lista,self.titulo_bar_model_map,self.rfont_titulo_bar_model_map,self.la[0],self.la[1],self.lo[0],self.lo[1],self.var_barra_loading,self.var_barra_loading_lbl, ))
		# thread_MAPA.start()
#####################|Thread Mapa|#######################################################################################################################################
#####################|Thread Mapa|#######################################################################################################################################
...
#####################|Thread Mapa|#######################################################################################################################################
	...
	# def animate_contour_MAPA(i):
	# 	self.img_axes.clear()
	# 	self.img_axes.contourf(DATA[:,:,i])
	# 	self.img_axes.set_title('%03d'%(i))
...
# 			self.img_axes.tick_params(axis='y', which='minor', width=float(L_MN_y),size=float(A_MN_y))#,labelsize=float(tam_y))
...
# 			self.img_axes.tick_params(axis='x', which='minor', width=float(L_MN_x),size=float(A_MN_x))#,labelsize=float(tam_x))
...
# 			# if self.rfont_titulo_axe_y_rot:
# 			# 	for ax in self.img_axes:
# 			# 		ax[0].set_ylabel(self.titulo_axe_y_rot,**self.rfont_titulo_axe_y_rot)
# 			# elif self.titulo_axe_y_rot != titulo_atual:
# 			# 	for ax in self.img_axes:
# 			# 		ax[0].set_ylabel(self.titulo_axe_y_rot)
...
#####################|Configurações do gráfico|#######################################################################################################################################
	...
	def Set_map(self,*event):
		...
		# print(self.map.set_llcrnrlat(-36))
		# self.fig_map, self.ax_map = plt.subplots()
		# self.map = Basemap(projection='cyl', llcrnrlat=-35, urcrnrlat=7,llcrnrlon=-
		# 77, urcrnrlon=-32, resolution='c')
		# self.map = Basemap(projection='cyl', llcrnrlat=int(la[0]), urcrnrlat=int(la[1]),llcrnrlon=int(lo[0]), urcrnrlon=int(lo[1]), resolution='c')
		# pass

	# def Set_menu_rap(self,*event):
	# 	self.Menu = True
	# 	# print(self.listaobs.get(0).split(','))
	...
	# 	self.un_sel(self.current_plot)#self.lista_plots[self.uti.list_duplicates_of(self.lista_plots_label,self.listaobs.get(item_id))[0]])
	...
	# 	self.ano = self.uti.refresh_list(self.ano,float(datetime.strptime("01/08/2018", '%d/%m/%Y').year),self.listaobs,False)
	# 	# self.uti.controle_entry_calendar(self.daData,self.cal2_INDV)
	...
	# 		self.clear_list_and_plot()
	# 		self.filedir = file
	# 		# nt_lista,conteudo_lista = self.uti.Refresh_list_obs(self.filedir,int(self.Data_Entry.get()[-4:]),self.check_filter_v.get())
	# 		if self.Menu: dip = True
	# 		nt_lista,self.conteudo_lista = self.uti.Refresh_list_obs(self.filedir,int(self.Data_Entry.get()[-4:]),self.check_filter_v.get(),dip)
	# 		# nt_lista,conteudo_lista = self.uti.Refresh_list_obs(self.filedir,int(self.Data_Entry.get()[-4:]),True)
	...
	# 		except (AttributeError,ValueError):
	# 			pass
	# 		if self.conteudo_lista:#or nt_lista:
	# 			self.lista_plots = []
	...
	# 				N_L = item_line[0].split(' ')
	# 				lat = (item_line[1].strip()).split(' ')
	# 				Lat_c = self.uti.DMDDEC(lat[0],lat[1])
	# 				Long_c = self.uti.DMDDEC(N_L[1],N_L[2])
	...
	# 			self.cbg.config(values=['Individual (STD)','Setor (STD)','EIA (STD)','Desvio (STD)','Mapa (CMN)','ROT (CMN)'])#'Mapa (CMN)'
