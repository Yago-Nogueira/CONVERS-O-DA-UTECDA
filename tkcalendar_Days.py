...
class Calendar(ttk.Frame):
	...
	def __init__(self, master=None, **kw):
		...
		if (("month" in kw) or ("year" in kw)) and ("day" not in kw):
			...
			try:
				...
				self._sel_date.append(self.date(year, month, day))  # selected day
			except ValueError:
				# self._sel_date = None
				del self._sel_date[:]
		...
		#print(self._cal.formatweekheader(3).split())
		...
		for i_week in range(6):
			...
			for i_day in range(7):
				...
				self._calendar[i_week][i_day].configure(text=txt,
														style=style)
		# self._display_selection(self.date(year, month, self._date.day))
		self._display_selection()

	def _display_selection(self):
		"""Highlight selected day."""
		for x in self._sel_date:
			if x is not None:
				year = x.year
	...
	def _remove_selection(self,x):
		...
		if x is not None:
			...
			del self._sel_date[(self._sel_date.index(x))]
	# --- callbacks
...
#            self._date = self._date.replace(year=year)
	...
	def _on_click(self, event):
		...
		# print(style)
		# print([self._style_prefixe + ".normal_om.TLabel", self._style_prefixe + ".we_om.TLabel"])
		...
		if day:
			...
			data_test = self.date(year, month, day)
			if data_test not in self._sel_date:
				self._sel_date.append(data_test)
				self._display_selection()
			else:
				self._remove_selection(data_test)
				# self._display_selection()
	...
	# 				# self._sel_date = date
	# 				self._sel_date.append(date)
	# 			else:
	# 				try:
	# 					# self._sel_date = self.strptime(date, "%x")
	# 					self._sel_date.append(self.strptime(date, "%x"))
