...
class IGRF12():
	def __init__(self):
		self.testesss = []
	def IGRF12SYN(self,isv,date,itype,alt,colat,elong,x,y,z,f):
		#global self.gh
########################LER MATRIZ coef ########################################        
		# arquivo=[]
		# self.gh=[0.0]
...
################################################################################
		...
		###########_G6_############
			...
			if m == 0:#goto 9
			###########_G9_############
				x[0] = x[0] + one * self.q[k]
				z[0] = z[0] - (fn + 1.0)*one*self.p[k]
				l = l + 1
			###########################
			else:
				two = (tc*self.gh[int(lm+1)] + t*self.gh[int(lm+nc+1)])*rr
				three = one*self.cl[m] + two*self.sl[m]
				x[0] = x[0] + three*self.q[k]
				z[0] = z[0] - (fn + 1.0)*three*self.p[k]
				if st == 0.0:#goto7
				###########_G7_#############
					y[0] = y[0] + (one*self.sl[m] - two*self.cl[m])*self.q[k]*ct
				############################
				else:
					y[0] = y[0] + (one*self.sl[m] - two*self.cl[m])*fm*self.p[k]/st
				############_G8_############
				l = l + 2#goto8
				############################
			###########_G10_############    
		...
		x[0] = x[0] * cd + z[0] * sd
		z[0] = z[0] * cd - one * sd
		f[0] = math.sqrt(x[0] * x[0] + y[0] * y[0] + z[0] * z[0]) 
		#f[0] =1.0# ('%.f')
		#return 
				#else:
	...
	def DMDDEC(self,I, M, X):
		...
		X[0] = DE + EM/60.0
#                               -7  46 -37 38
	def get_igrf(self,DATE,ALT,LTD,LTM,LND,LNM):
		ITYPE = 1
		XLT = [0]
		XLN = [0]
		X=[0]
		...
		# DATE =float(input('Digite o ano:'))
		#ALT =float(input('Digite a altitude em km:'))
		# print('Digite a latitude e longitude:')
		#LTD,LTM,LND,LNM =int(input()),int(input()),int(input()),int(input())
		# print('\n')
		self.DMDDEC(LTD, LTM, XLT)
		self.DMDDEC(LND, LNM, XLN)
		CLT = 90.0 - XLT[0]#ok
		self.IGRF12SYN(0, DATE, ITYPE, ALT, CLT, XLN, X, Y, Z, F)
		FACT = 57.295776367187500#180.0/3.141592654
		#fortran ->>dupla precisão (64 bits).
		
		#---DIVERGE---#
		#57.2957795056
		#RESULTADO IGRF12 fortran = 57.295776367187500
		#RESULTADO IGRF12 python  = 57.29577950560105

		#print(FACT)        


		D = FACT * math.atan2(Y[0], X[0])
		H = math.sqrt(X[0]*X[0] + Y[0]*Y[0])   
		S = FACT * math.atan2(Z[0], H)

		self.DDECDM(D, IDEC, IDECM)
		self.DDECDM(S, INC, INCM)
		


		self.IGRF12SYN(1, DATE, ITYPE, ALT, CLT, XLN, DX, DY, DZ, F1)
		DD = (60.0*FACT*(X[0]*DY[0] - Y[0]*DX[0]))/(H*H)
		DH = (X[0]*DX[0] + Y[0]*DY[0])/H
		DS = (60.0*FACT*(H*DZ[0] - Z[0]*DH))/(F[0]*F[0])
		DF = (H*DH + Z[0]*DZ[0])/F[0]
		IDS = round(DS)
		


		#print(INC[0])
		#print('I =' + str(INC[0]) + ' deg ' + str(INCM[0]) + ' min ' + 'SV = '+ str(IDS) + ' min/yr' )
		return (str(INC[0])+" "+ str(INCM[0]))#('I = ' + str(INC[0]) + ' deg ' + str(INCM[0]) + ' min ')
if __name__ == "__main__":
	incli = IGRF12()
	res_incli = incli.get_igrf(1998,300,-7,46,-37,38)
	print(res_incli)