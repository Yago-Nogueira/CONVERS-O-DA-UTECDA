...
class IGRF12():
	def get_igrf(self):
		...
		self.IDEC=0;self.IDECM=0 ;self.INC=0; self.INCM=0
		self.FACT = 180.0/3.141592654
		self.DTMN = 1900.0
		self.DTMX = 2025.0
		self.DATE = 1998.0
		self.ALT=300.0
		...
		self.LND=-37
		self.LNM=38
		if self.LTD < -90 or self.LTD > 90 or self.LTM <= -60 or self.LTM >= 60:print(' ***** Error ***** Latitude out of range') #GO TO 204
		if self.LND < -360 or self.LND > 360 or self.LNM <= -60 or self.LNM >= 60:print('  ***** Error ***** Longitude out of range')#GO TO 205
		if self.LTM < 0 and self.LTD != 0:print(' ***** Error ***** Latitude out of range')# GO TO 204
		if self.LNM < 0 and self.LND != 0:print(' ***** Error ***** Longitude out of range')# GO TO 205
		if self.DATE < self.DTMN or self.DATE > self.DTMX:print(' ***** Error ***** DATE Extrapolationsss')# GO TO 209
		self.XLT = self.DMDDEC(self.LTD,self.LTM)
		self.XLN = self.DMDDEC(self.LND,self.LNM)
		self.CLT = 90.0 - self.XLT
		if self.CLT < 0.0 or self.CLT > 180.0:print(' ***** Error ***** Latitude out of range')
		if self.XLN <= -360.0 or self.XLN >= 360.0:print(' ***** Error ***** Longitude out of range')

		# print(self.CLT,self.XLN)......................OK
		self.X,self.Y,self.Z,self.F = self.IGRF12SYN(0,self.DATE,self.ALT,self.CLT,self.XLN)#,X,Y,Z,F)
		# self.D = self.FACT * math.atan2(self.Y, self.X)
		self.H = math.sqrt(self.X*self.X + self.Y*self.Y)   
		self.S = self.FACT * math.atan2(self.Z, self.H)
		# self.IDEC,self.IDECM = self.DDECDM(self.D)
		self.INC,self.INCM = self.DDECDM(self.S)
		# self.DX, self.DY, self.DZ, self.F1 = self.IGRF12SYN(1, self.DATE, self.ALT, self.CLT, self.XLN)



		# self.DD = (60.0*self.FACT*(self.X*self.DY - self.Y*self.DX))/(self.H*self.H)
		# self.DH = (self.X*self.DX + self.Y*self.DY)/self.H
		# self.DS = (60.0*self.FACT*(self.H*self.DZ - self.Z*self.DH))/(self.F*self.F)
		# self.DF = (self.H*self.DH + self.Z*self.DZ)/self.F
		# self.IDS = round(self.DD)
		...
		# self.IDS = round(DS)
		return 'oi'
	def IGRF12SYN(self,isv,date,alt,colat,elong):#,x,y,z,f):
		x = 0.0
		y = 0.0
		z = 0.0
		f = 0.0
...
########################LER MATRIZ coef ########################################        
		# arquivo=[]
		# self.gh=[0.0]
		# arq = open('igrf12coeffs.txt','r',encoding="UTF-8")
		# xma=0
		# jma=0
		# for line in arq:
		# 	if not line.startswith('g/h') and line[0] != '#' and line[0] != 'c' :
		# 		pil =  line.split()
		# 		del(pil[:3])
		# 		arquivo.append(pil)
		# for jma in range(len(arquivo[xma])):
		# 	for xma in range(len(arquivo)):
		# 		self.gh.append(float(arquivo[xma][jma]))
		# sma = 121
		# for k in range(19):
		# 	del(self.gh[sma:sma+75])
		# 	sma+=120
################################################################################
		self.cl = [0]*13
		self.sl = [0]*13
		self.q = [0]*105
		self.p = [0]*105
		self.gh=[0.0,-31543.,-2298., 5922., -677., 2905.,-1061.,  924., 1121., 
...
########################################################################################################################################################################      
		self.cl[1] = math.cos(one)
		self.sl[1] = math.sin(one)
		cd = 1.0
		sd = 0.0
		l = 1
		m = 1
		...
		###########_G4_############
			...
			if m == n and k != 3 :
				...
				j = k - n - 1
				self.p[k] = one*st*self.p[j]
				self.q[k] = one*(st*self.q[j] + ct*self.p[j])
				self.cl[m] = self.cl[m-1]*self.cl[1] - self.sl[m-1]*self.sl[1]
				self.sl[m] = self.sl[m-1]*self.cl[1] + self.cl[m-1]*self.sl[1]
			elif m != n:#goto 5
			###########_G5_############
				gmm = m*m
				...
				i = k - n
				j = i - n + 1
				self.p[k] = three*ct*self.p[i] - two*self.p[j]
				self.q[k] = three*(ct*self.q[i] - st*self.p[i]) - two*self.q[j]
		...
		###########_G6_############
			lm = ll + l
			one = (tc*self.gh[int(lm)] + t*self.gh[int(lm+nc)])*rr
			if m == 0:#goto 9
			###########_G9_############
				x = x + one * self.q[k]
				z = z - (fn + 1.0)*one*self.p[k]
				l = l + 1
			###########################
			else:
				two = (tc*self.gh[int(lm+1)] + t*self.gh[int(lm+nc+1)])*rr
				three = one*self.cl[m] + two*self.sl[m]
				x = x + three*self.q[k]
				z = z - (fn + 1.0)*three*self.p[k]
				if st == 0.0:#goto7
				###########_G7_#############
					y = y + (one*self.sl[m] - two*self.cl[m])*self.q[k]*ct
				############################
				else:
					y = y + (one*self.sl[m] - two*self.cl[m])*fm*self.p[k]/st
				############_G8_############
				l = l + 2#goto8
				############################
			###########_G10_############    
	...
	# def DDECDM(self,X, I, M):
	def DDECDM(self,X):
		SIG = 0#np.sign(1.10,X)
		if X>=0:
		   SIG = 1.1
		elif X<0:
		   SIG = -1.1
...
if __name__ == "__main__":
	x = IGRF12()
	print(x.get_igrf())#1998,300,-7,46,-37,38)
	# incli = 
	# print(x).get_igrf(1998,300,-7,46,-37,38)
	# res_incli = incli.
	# print(res_incli)