...
class COMP_MAPA(ui.Toplevel):	
	...
	def _set_Matplotlib_grafico(self):
		try:
			_titulo_bar = self.Dado_config.Settings["MAPA"]["sTitle_B_VTEC"]
	...
	#'____________________________________________________________________________________________________________________________'
			...
			self.list_prn = []
			nome_est = []
			passo = (10/(len(self._siglas)+1))
			for es in self._siglas:
				caminho = (("%s\%s%.3i-%s-%.2i-%.2i.Cmn") % (self._filedir,es[0].lower(),dia_ano,self._data.year,self._data.month,self._data.day))
				
				prns_cmn,dado_cmn = self.uti.Leitura_CMN(destino = caminho, ele = self._elevacao)
				if prns_cmn and dado_cmn:
					nome_est.append(es[0].lower())
					self.list_prn.append(prns_cmn)
					self.dados[es[0].lower()] = (dado_cmn)
					self._var_barra_progess.set(self._var_barra_progess.get() + passo)
	#'____________________________________________________________________________________________________________________________'
			self._var_barra_progess.set(20)
	#'____________________________________________________________________________________________________________________________'
			# print('_______________________________')
			passo = (20/(len(np.arange(self._h_i,self._h_f,(self._h_interval/60)))+1))
			self.dados_organizados = {}
			for hora in np.arange(self._h_i,self._h_f,(self._h_interval/60)):
				f_hora = ("%.6f"%hora)
				self.dados_organizados[f_hora+".vtec"] = []
				self.dados_organizados[f_hora+".lat"] = []
				self.dados_organizados[f_hora+".lon"] = []

				for sigla_est,_prn in zip(nome_est,self.list_prn):
					for prn in _prn:
						inds = self.uti.list_duplicates_of(self.dados[sigla_est][str(prn)+".time"],f_hora)
						for ind in inds:
							self.dados_organizados[f_hora+".vtec"].append(self.dados[sigla_est][str(prn)+'.vtec'][ind])
							self.dados_organizados[f_hora+".lat"].append(self.dados[sigla_est][str(prn)+'.lat'][ind])
							self.dados_organizados[f_hora+".lon"].append(self.dados[sigla_est][str(prn)+'.lon'][ind])
				self._var_barra_progess.set(self._var_barra_progess.get() + passo)
	#'____________________________________________________________________________________________________________________________'
			self._var_barra_progess.set(40)
			self._var_barra_progess_label.set(self._dado_config.idioma(178) +str(self._data.year))
	#'____________________________________________________________________________________________________________________________'
			passo = (10/(len(range(-180,181,1))+1))
			level = np.arange(0,(self._vm+1),1)
			numcols, numrows = 40,40
			# eq_y = []
			# eq_x = []
			# for x in range(-180,181,1):
			# 	inclinacao = self.uti.get_inclinacao(300,self._data.year,0,0,x,0)
			# 	diplat = -(math.degrees(math.atan(((math.tan(math.radians(float(inclinacao)))/2)))))
			# 	eq_y.append(diplat)
			# 	eq_x.append(x)
			# 	self._var_barra_progess.set(self._var_barra_progess.get() + passo)
	#'____________________________________________________________________________________________________________________________'
			self._var_barra_progess.set(50)
			self._var_barra_progess_label.set(self._dado_config.idioma(179))
	#'____________________________________________________________________________________________________________________________'
			try:
				pasta = ("Contorno%s-%.2i-%.2i"%(self._data.year,self._data.month,self._data.day))
				os.makedirs(self._filedir+"\\"+pasta)
			except FileExistsError:
				pass
			
			passo = (50/(len(np.arange(self._h_i,self._h_f,(self._h_interval/60)))+1))
			for hora in np.arange(self._h_i,self._h_f,(self._h_interval/60)):
				# self.axes.cla()
				self.fig.clf()
				...
				f_hora = ("%.6f"%hora)
				self.axes.set_title(hora_HMS,**font)
				self.axes.set_extent(self._extend_LAT_LONG)
				self.axes.add_feature(cfeature.BORDERS)
				self.axes.add_feature(cfeature.COASTLINE)
				x, y, z = np.array(self.dados_organizados[f_hora+'.lon']), np.array(self.dados_organizados[f_hora+'.lat']), np.array(self.dados_organizados[f_hora+'.vtec']) 
				...
				# self.axes.plot(eq_x,eq_y,'K',gid="line_ecuador_mag",picker=2)
				self.axes.plot(self._Coordenadas_equador_Magnetico_X,self._Coordenadas_equador_Magnetico_Y,'K')#gid="line_ecuador_mag",picker=2)
				cbar_mapa = self.fig.colorbar(self.axes.contourf(xi,yi,GD1, cmap = cmap,levels = level, vmin = 0, vmax = self._vm ,extend="both",transform=ccrs.PlateCarree()),ticks = np.arange(0,self._vm+1,5))
				# self.axes.contourf(xi,yi,GD1,cmap = cmap,levels= level, vmin = 0, vmax = self._vm ,extend="both")
				# cbar_mapa = self.axes.colorbar(ax = self.axes,ticks = np.arange(0,self._vm+1,5))
				cbar_mapa.ax.set_title(_titulo_bar,**font)
				# except (ValueError) as e:
					# print(e)
					# self.fig.text(.5,.5, self._dado_config.idioma(94), ha="center", va="center" )
				camff = (("%s\%s\%s.png")%(self._filedir,pasta,hora_HMS.replace(':',"-")))
				
				
				self.fig.savefig(camff,dpi=self.fig.dpi)
				self._var_barra_progess_label.set("Figura - "+camff+" gerada")
				self._var_barra_progess.set(self._var_barra_progess.get() + passo)
				# plt.savefig( (("%s\%s.png")%(self._filedir,hora.replace(':',"-")))  ,dpi=self.fig.dpi)
	#'____________________________________________________________________________________________________________________________'
			self._var_barra_progess.set(100)
			self._var_barra_progess_label.set(self._dado_config.idioma(180))
	#'____________________________________________________________________________________________________________________________'
			plt.close()
			return None#,self.list_figs,self.list_titles
		except (RuntimeError,SystemError):#,Exception):
			print('sairr')
			# return None#,self.list_figs,self.list_titles
